...
export class BackgroundImageDirective implements OnInit, OnChanges, OnDestroy {
  ...
  public ngOnInit(): void {
    this._dataUrl$.pipe(takeUntil(this._destroy$)).subscribe((dataUrl) => {
      if (dataUrl) {
        const safeUrl = this._domSanitizer.sanitize(
          SecurityContext.URL,
          dataUrl
        );
        console.log(safeUrl);
  ...
  private _loadImage(src: string | null): Observable<any> {
    if (src !== null) {
      return this._httpClient
        ...
        .pipe(
          map((e) =>
            this._domSanitizer.bypassSecurityTrustUrl(URL.createObjectURL(e))
          )
        );
    }
